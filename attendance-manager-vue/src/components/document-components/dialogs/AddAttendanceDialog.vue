<template>
    <v-card>
        ...
        <v-card-text>
            <v-btn class="blue-grey lighten-2 ma-2"
                   @click="dialog2 = !dialog2"
                   disabled>
                Upload attendance
            </v-btn>
            <v-btn class="blue-grey lighten-2 ma-2"
                   @click="generateCodeDialog = !generateCodeDialog">
                Generate code
            </v-btn>
            <v-dialog v-if="generateCodeDialog"
                      v-model="generateCodeDialog"
                      width="50%"
                      :fullscreen="isMobile">
                <GenerateAttendanceCodeDialog @close="generateCodeDialog = false" />
            </v-dialog>
            <v-btn class="blue-grey lighten-2 ma-2"
                        ...
                        <tr v-for="item in students" :key="item.attendanceID">
                            ...
                            <td>
                                <v-text-field type="number"
                                              style="width: 40px"
                                              v-model="item.bonusPoints"></v-text-field>
                            </td>
                        </tr>
                    </tbody>
                </template>
...
<script lang="ts">
    ...
    export default Vue.extend({
        ...
        methods: {
            ...
            onCloseGenerateCodeDialog: function (): void {
                console.log("done");
